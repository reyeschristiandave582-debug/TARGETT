 ...
 * Includes iOS safe area top spacing, dynamic urgency timer, and optimized toast placement.
 */
const AnnouncementBar = () => {
  ...
  // Live social proof toast rotation
  useEffect(() => {
  ...
  return (
    <>
      {/* Top Banner Bar with Explicit Top Padding for iOS Status Bar */}
      <div className="sticky top-0 z-50 w-full bg-[#CC0000] border-b border-[#A00000] pt-10 sm:pt-[calc(env(safe-area-inset-top)+10px)] pb-2 px-3 sm:px-4 shadow-[0_4px_20px_rgba(0,0,0,0.3)] backdrop-blur-md">
        {/* Background Sparkles */}
      ...
      {/* Social Proof Floating Toast (Positioned safely above CTA button) */}
      {currentNotif && (
        <div
          className={`fixed bottom-[110px] sm:bottom-6 left-3 right-3 sm:left-4 sm:right-auto z-[9999] max-w-sm mx-auto sm:mx-0 flex items-center gap-2.5 rounded-xl border-l-[4px] border-[#CC0000] bg-white/98 backdrop-blur-md px-3.5 py-2.5 shadow-[0_12px_30px_rgba(0,0,0,0.22)] overflow-hidden transition-all duration-500 ease-in-out ${
            isVisible
